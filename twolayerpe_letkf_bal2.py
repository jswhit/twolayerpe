...
from enkf_utils import cartdist,letkf_update,gaspcohn
...
# this version uses different LETKF weights for balanced and unbalanced part
...
print("# hcovlocal=%g hcovlocal_unbal=%g baldiv=%s  covinf1=%s covinf2=%s nanals=%s" %\
     (hcovlocal_scale/1000.,hcovlocal_scale_unbal/1000.,baldiv,covinflate1,covinflate2,nanals))
...
def gethofxmean(uensmean,vensmean,zmidensmean,indxob,nanals,nobs):
    hxensmean = np.empty(5*nobs,dtype)
    hxensmean[0:nobs] = uensmean[0,...].ravel()[indxob] # interface height obs
    hxensmean[nobs:2*nobs] = vensmean[0,...].ravel()[indxob] # interface height obs
    hxensmean[2*nobs:3*nobs] = uensmean[1,...].ravel()[indxob] # interface height obs
    hxensmean[3*nobs:4*nobs] = vensmean[1,...].ravel()[indxob] # interface height obs
    hxensmean[4*nobs:] = zmidensmean[:].ravel()[indxob] # interface height obs
    return hxensmean
...
for ntime in range(nassim):
    ...
    hxensmean = gethofxmean(uensmean,vensmean,ztop-dzensmean[1,...],indxob,nanals,nobs)
    hxens_bal = gethofx(uens_bal,vens_bal,ztop-dzens_bal[:,1,...],indxob,nanals,nobs)
    hxens_bal = hxens_bal - hxens_bal.mean(axis=0) + hxensmean # recenter
    hxens_unbal = gethofx(uens_unbal,vens_unbal,ztop-dzens_unbal[:,1,...],indxob,nanals,nobs)
    hxens_unbal = hxens_unbal - hxens_unbal.mean(axis=0) + hxensmean # recenter
    ...
    if not debug_model: 
        ...
        xens = letkf_update(xens,hxens_bal,obs,oberrvar,covlocal_tmp,n_jobs,nlevs_update=nlevs_update) 
        t2 = time.time()
    ...
    if hcovlocal_scale_unbal > 1001: # otherwise don't update unbalanced part.
        ...
        # update state vector with serial filter or letkf.
        if not debug_model: 
            xens = letkf_update(xens,hxens_unbal,obs,oberrvar,covlocal_tmp,n_jobs)
            t2 = time.time()
            if profile: print('cpu time for EnKF update',t2-t1)
            xensmean_a = xens.mean(axis=0)
            xprime = xens-xensmean_a
    ...
    if posterior_stats:
        ...
        zsfc_errav_a,zsfc_sprdav_a,zmid_errav_a,zmid_sprdav_a,ke_errav,ke_sprdav=\
        getspreaderr(model,uens,vens,dzens,\
        u_truth[ntime+ntstart],v_truth[ntime+ntstart],dz_truth[ntime+ntstart],ztop)
