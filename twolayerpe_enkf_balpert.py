...
print("# hcovlocal=%g  baldiv=%s  use_letkf=%s covinf1=%s covinf2=%s nanals=%s" %\
     (hcovlocal_scale/1000.,baldiv,use_letkf,covinflate1,covinflate2,nanals))
...
def balenspert(model,uens,vens,dzens,div=False,nitermax=500,divguess=False,relax=0.015,eps=1.e-2,verbose=False):
    if div==True:
        div=None
    nanals = uens.shape[0]
    uenspert_bal = np.empty(uens.shape, uens.dtype)
    venspert_bal = np.empty(uens.shape, uens.dtype)
    dzenspert_bal = np.empty(uens.shape, uens.dtype)
    uensmean = uens.mean(axis=0)
    vensmean = vens.mean(axis=0)
    dzensmean = dzens.mean(axis=0)
    vrtspec, divspec = model.ft.getvrtdivspec(uensmean,vensmean)
    vrtensmean = model.ft.spectogrd(vrtspec)
    divensmean = model.ft.spectogrd(divspec)
    for nmem in range(nanals):
        vrtspec, divspec = model.ft.getvrtdivspec(uens[nmem],vens[nmem])
        vrt = model.ft.spectogrd(vrtspec)-vrtensmean
        dzbal, divbal = getincbal(model,dzensmean,vrtensmean,divensmean,vrt,div=div,nitermax=nitermax,\
                        relax=relax,eps=eps,verbose=verbose)
        if div==False:
            # no balanced divergence (much faster)
            divspec = np.zeros(vrtspec.shape, vrtspec.dtype)
        else:
            divspec = model.ft.grdtospec(divbal)
        uenspert_bal[nmem], venspert_bal[nmem] = model.ft.getuv(vrtspec,divspec)
        dzenspert_bal[nmem] = dzbal
    return uenspert_bal,venspert_bal,dzenspert_bal
...
for ntime in range(nassim):
    ...
    uenspert_bal,venspert_bal,dzenspert_bal = balenspert(model,uens,vens,dzens,baldiv=baldiv)
    uensmean = uens.mean(axis=0)
    vensmean = vens.mean(axis=0)
    dzensmean = dzens.mean(axis=0)
    uenspert_unbal = (uens-uensmean)-uenspert_bal
    venspert_unbal = (vens-vensmean)-venspert_bal
    dzenspert_unbal = (dzens-dzensmean)-dzenspert_bal

    # compute forward operator.
    # hxens is ensemble in observation space.
    hxens = gethofx(uensmean_b+uenspert_bal,vensmean_b+venspert_bal,ztop-(dzensmean_b+dzenspert_bal)[:,1,...],indxob,nanals,nobs)
    ...
    # EnKF update (using balance ens perts).
    xens = enstoctl(model,uensmean+uenspert_bal,vensmean+venspert_bal,dzensmean+dzenspert_bal,ivar=ivar)
    ...
    vensmean = vens.mean(axis=0)
    dzensmean = dzens.mean(axis=0)
    # add unbalanced perts from background back in.
    uens = uensmean + uenspert_bal + uenspert_unbal
    vens = vensmean + venspert_bal + venspert_unbal
    dzens = dzensmean + dzenspert_bal + dzensert_unbal
