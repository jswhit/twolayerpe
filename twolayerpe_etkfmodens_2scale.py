...
from enkf_utils import cartdist,gaspcohn,getkf,modensv,gethofxv
...
oberrstdev_zmid = np.sqrt(1.e30) # surface height ob error in meters
...
for ntime in range(nassim):
    ...
    if nobs == Ntsq: # observe every grid point
        indxob = np.arange(Ntsq)
        ...
        indxob = rsobs.choice(Ntsq,nobs,replace=False)
    ...
    xprime_b = enstoctl(upert_b,vpert_b,dzpert_b)
    # compute forward operator, modulated ensemble
    hxensmean = gethofxv(xensmean_b,indxob,0,nobs,ztop=ztop)
    hxprime_b = gethofxv(xprime_b,indxob,nanals,nobs)
    xprime_bm = modensv(xprime_b,sqrtcovlocal)
    hxprime_bm = gethofxv(xprime_bm,indxob,nanals2,nobs)
