...
import sys, time, os
from twolayer import TwoLayer, run_model, run_model_iau, run_model4d
...
masstend_diag = 0.
uens4d = np.empty((assim_timesteps,nanals,2,model.ft.Nt,model.ft.Nt),model.ft.dtype)
vens4d = np.empty((assim_timesteps,nanals,2,model.ft.Nt,model.ft.Nt),model.ft.dtype)
dzens4d = np.empty((assim_timesteps,nanals,2,model.ft.Nt,model.ft.Nt),model.ft.dtype)
...
for ntime in range(nassim):
    ...
    def update(model,uens,vens,dzens,wts,covinflate1,covinflate2,\
               fix_totmass=True,profile=False):
        nanals = uens.shape[0]
    ...
    if ntime == 0 or posterior_stats:
        uens_mid_a,vens_mid_a,dzens_mid_a,uens_inc_mid,vens_inc_mid,dzens_inc_mid = \
               ...
               fix_totmass=fix_totmass,profile=profile)
    ...
    if ntime == 0:
        ...
        if fix_totmass:
            for nmem in range(nanals):
                dzens[nmem][0] = dzens[nmem][0] - dzens[nmem][0].mean() + model.zmid
                dzens[nmem][1] = dzens[nmem][1] - dzens[nmem][1].mean() + model.ztop - model.zmid
        results = Parallel(n_jobs=n_jobs)(delayed(run_model4d)(uens[nanal],vens[nanal],dzens[nanal],N,L,dt,assim_timesteps,theta1=theta1,theta2=theta2,zmid=zmid,ztop=ztop,diff_efold=diff_efold,diff_order=diff_order,tdrag=tdrag,tdiab=tdiab,umax=umax,jetexp=jetexp,div2_diff_efold=div2_diff_efold) for nanal in range(nanals))
        masstend_diag=0.
        for nanal in range(nanals):
            uens4d[:,nanal,...],vens4d[:,nanal,...],dzens4d[:,nanal,...],mtend = results[nanal]
            masstend_diag+=mtend/nanals
        uens_beg=uens4d[0]; vens_beg=vens4d[0]; dzens_beg=dzens4d[0]
        uens_mid=uens4d[assim_timesteps//2]; vens_mid=vens4d[assim_timesteps//2]; dzens_mid=dzens4d[assim_timesteps//2]
        if fix_totmass:
            for nmem in range(nanals):
                dzens_beg[nmem][0] = dzens_beg[nmem][0] - dzens_beg[nmem][0].mean() + model.zmid
                dzens_beg[nmem][1] = dzens_beg[nmem][1] - dzens_beg[nmem][1].mean() + model.ztop - model.zmid
                dzens_mid[nmem][0] = dzens_mid[nmem][0] - dzens_mid[nmem][0].mean() + model.zmid
                dzens_mid[nmem][1] = dzens_mid[nmem][1] - dzens_mid[nmem][1].mean() + model.ztop - model.zmid
        ...
        tstart = model.t
        # compute analysis at every time step using LETKF wts
        for nt in range(assim_timesteps):
            xens = enstoctl(model,uens4d[nt],vens4d[nt],dzens4d[nt])
            xensmean_b = xens.mean(axis=0)
            xprime = xens-xensmean_b
            fsprd = (xprime**2).sum(axis=0)/(nanals-1)
            for k in range(xens.shape[1]):
